...
# In[2]:Import
abcd=pd.read_csv("C:/Users/hp/Desktop/Leukaemia.csv")
...
test2=SelectKBest(score_func=mutual_info_regression)
...
###########Cascading Filtering First

ttest2=stats.ttest_ind(X1,Y1)
...
S6.sort()
top6=[]
for d in range(0,700):
    top6.append(S6[699-d])
W6=[]
...
final2 = abcd.iloc[:,W6]
test7=SelectKBest(score_func=mutual_info_regression)
fit7=test7.fit(final2,Y1)
r7=fit7.scores_
l7=[]
s7=fit7.scores_
...
sfs = SFS(neigh7,k_features=10,forward=True,floating=False,scoring='accuracy',cv=0)
sfs=sfs.fit(X1,Y1)
